...
class StoreMovimento extends FormRequest
{
    ...
    {
        return $this->user()->direcao || $this->user()->tipo_socio == 'P';
    }
    ...
    public function rules()
    {
        return [
            ...
            'piloto_id' => ['required','integer','digits_between:1,20',Rule::exists('users', 'id')->where(function($query) {
                ...
                if($this->user()->direcao != '1' && ($value!=$this->user()->id && $this->instrutor_id != $this->user()->id)) {
                    $fail('Não pode criar um movimento em que não seja Piloto ou Instrutor.');
                }
            },
        ],
            ...
            'instrutor_id' => ['nullable','integer','digits_between:1,20','required_if:natureza,I','different:piloto_id',Rule::exists('users', 'id')->where(function($query) {
                ...
                if($this->user()->direcao != '1' && ($value!=$this->user()->id && $this->piloto_id != $this->user()->id)) {
                    $fail('Não pode criar um movimento em que não seja Instrutor ou Piloto.');
                }
            },
        ],
    ...
    public function messages()
    {
        return [
            ...
            'aeronave.exists' => 'Piloto/Instrutor não autorizado(s) a voar na Aeronave. Preencha com uma Aeronave válida.',
            ...
            'conta_horas_fim.gte' => 'Preencha o campo Conta-Horas Final com um valor superior ao campo Conta Horas Inicial.',
